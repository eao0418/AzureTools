...
#pragma warning restore CS8604 // Possible null reference argument.
        ...
        public async Task SetSecretAsync(string name, string value)
        {
            ...
            var vaultUri = _providerSettings.KeyVaultUri ?? throw new Exception($"Key Vault URI cannot be null or empty. {nameof(_providerSettings.KeyVaultUri)}");
            var client = InitializeClient(vaultUri);

            var secret = await client.SetSecretAsync(name, value) ?? throw new Exception($"Failed to set secret '{name}' in Key Vault at {vaultUri}.");

            var rawResponse = secret.GetRawResponse() ??
                throw new Exception($"Failed to set secret '{name}' in Key Vault at {vaultUri}."); // Ensure the secret is set by accessing the response
